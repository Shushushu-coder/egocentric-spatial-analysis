...
```text
video → preprocessing → reconstruction → viewpoint-aware spatial analysis
```

Two or more time-overlapping egocentric videos can be processed as labeled observers. Each observer is a named input. The software never assigns identity from frame index parity.
...
3. **Sample frames** in a time window using `uniform`, `quality_weighted`, or `complementary` strategies. Quality combines Laplacian sharpness, brightness, contrast, and ORB feature count, with penalties for blur and bad exposure.
...
- **This project:** capture-oriented preprocessing, memory-aware wrapping, and labeled viewpoint comparisons on those artifacts.
